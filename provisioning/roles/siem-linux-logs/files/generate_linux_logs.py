...
# Give logcollector time to notice the new, empty files.
time.sleep(45)

BATCH = 10
written = {name: 0 for name in files}
while any(written[n] < len(files[n][1]) for n in files):
    for name, (path, lines) in files.items():
        start_i = written[name]
        if start_i >= len(lines):
            continue
        chunk = lines[start_i:start_i + BATCH]
