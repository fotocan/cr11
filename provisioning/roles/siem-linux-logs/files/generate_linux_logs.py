...
# ----------------------------------------------------------------------
def keepalive(name):
    """A plausible routine line, used to keep a file being written to while
    logcollector works through the others. These are indistinguishable from the
    rest of the generated content — ordinary operations, no attacker activity."""
    t = datetime.now(timezone.utc)
    if name == "auth":
        return f"{ts(t)} {HOST} sshd[{random.randint(1000,9999)}]: Received disconnect from {ADMIN_SUBNET}{random.randint(20,60)} port {random.randint(40000,60000)}:11: disconnected by user"
    if name == "audit":
        return f"type=SYSCALL msg=audit({audit_ts(t)}): arch=c000003e syscall=257 success=yes exit=3 auid=1004 uid=1004 comm=\"rsync\" exe=\"/usr/bin/rsync\" key=\"share-access\""
    return f"{ts(t)} {HOST} CRON[{random.randint(1000,9999)}]: (root) CMD (command -v debian-sa1 > /dev/null && debian-sa1 1 1)"


# ----------------------------------------------------------------------
# Write by APPENDING slowly rather than in one pass.
#
# wazuh-logcollector opens a monitored file and seeks to the END. Anything
# already in the file when it starts watching is never read — not on open,
# not on a manager restart. Writing all 159 lines at once therefore produces
# a file full of events that never reach the index, which is exactly what
# happened for several builds.
#
# Creating the files empty first, then appending in small batches with a
# pause between them, means logcollector is already watching and each batch
# arrives as new data. The pauses total roughly two minutes.
# ----------------------------------------------------------------------
import time
...
for name, lines in (("auth", auth), ("syslog", syslog), ("audit", audit)):
    ...
    open(path, "w").close()          # create empty, so logcollector starts at 0
    files[name] = (path, lines)
    print(f"{path}: {len(lines)} lines to write")

# Give logcollector plenty of time to notice the new, empty files. This is
# deliberately generous: the cost is build time, and the failure it prevents is
# silent — a file logcollector opens after we finish writing is a file whose
# contents never reach the index at all.
time.sleep(90)

# Spread every file over the SAME number of rounds rather than the same batch
# size. The first attempt used a fixed batch of 10, so the 113-line syslog kept
# writing for over a minute while the 23-line auth and audit files finished in
# under twenty seconds — and logcollector, which opens monitored files one at a
# time, had already missed them. Equal duration means every file is still being
# written whenever logcollector gets to it.
ROUNDS = 45
written = {name: 0 for name in files}
for r in range(ROUNDS):
    for name, (path, lines) in files.items():
        target = round(len(lines) * (r + 1) / ROUNDS)
        chunk = lines[written[name]:target]
        if chunk:
    ...
    # Every file gets at least one line every round, even after its real
    # content is exhausted. A file that stops being written to before
    # logcollector opens it is a file that never gets read, and the shorter
    # logs would otherwise fall silent long before the longest one finishes.
    for name, (path, lines) in files.items():
        if written[name] >= len(lines):
            with open(path, "a") as fh:
                fh.write(keepalive(name) + "\n")
                fh.flush()

# Anything left over from rounding.
for name, (path, lines) in files.items():
    chunk = lines[written[name]:]
    if chunk:
        with open(path, "a") as fh:
            fh.write("\n".join(chunk) + "\n")
...
for name, (path, lines) in files.items():
    print(f"{path}: {written[name]} lines appended")
